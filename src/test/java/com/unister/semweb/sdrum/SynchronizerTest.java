...
public class SynchronizerTest {
    ...
    public void updateElements() throws Exception {
        ...
        toUpdate.setKey(toUpdateOriginal.key);
        ...
        DummyKVStorable[] readLinkData = readInFile(testFilename);
        DummyKVStorable searchResult = TestUtils.searchFor(readLinkData, toUpdate.key);
        if (searchResult == null) {
            Assert.fail();
        } else {
            DummyKVStorable expectedLinkDate = new DummyKVStorable();
            expectedLinkDate.setKey(toUpdateOriginal.key);
            expectedLinkDate.setParentCount(toUpdateOriginal.getParentCount() + 1);
            expectedLinkDate.setRelevanceScore(toUpdateOriginal.getRelevanceScore() * toUpdateOriginal.getParentCount()
                    + toUpdate.getRelevanceScore() / (toUpdateOriginal.getParentCount() + 1));

            Assert.assertEquals(toUpdateOriginal.key, searchResult.key);
    ...
    private DummyKVStorable[] readInFile(String filename) throws IOException, FileLockException {
        ...
        HeaderIndexFile<DummyKVStorable> file = new HeaderIndexFile<DummyKVStorable>(filename, AccessMode.READ_WRITE,
                100, prototype.keySize, prototype.byteBufferSize);
        long offset = 0;
