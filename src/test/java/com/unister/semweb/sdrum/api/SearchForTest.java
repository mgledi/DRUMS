...
/**
 * Tests the <code>select</code> method of the {@link SDRUM} class.
 * 
 * @author n.thieme
 * 
 */
public class SearchForTest {
    ...
    /** Some data are stored within the {@link SDRUM} and the keys of these data are retrieved. */
    @Test
    public void searchForKnownKeys() throws Exception {
        ...
        SDRUM<TestStorable> sdrum = SDRUM_API.createOrOpenTable(hashFunction, globalParameters);
        ...
        SDRUM<TestStorable> searchSdrum = SDRUM_API.createOrOpenTable(hashFunction, globalParameters);
        List<TestStorable> readData = searchSdrum.select(keysToSearchFor);
        Assert.assertEquals(testdata.length, readData.size());
        Assert.assertTrue(equals(readData, testdata));
    }

    /**
     * Some data are written to the {@link SDRUM} but only unknwon keys keys are searced for. Nothin must be returned by
    ...
    /** Some data are written to the {@link SDRUM}, known and unknown keys are searched for. */
    @Test
